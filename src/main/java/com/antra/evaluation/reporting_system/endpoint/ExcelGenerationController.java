...
//        this.repo = this.excelService.getRepo();
    ...
    public ResponseEntity<ExcelResponse> createExcel(@RequestBody @Validated ExcelRequest request) throws IOException {
        ...
        response.setResponse(response.getResponse()+filename+"aaaaaaaaaaaaaaaaa");
        if (f.exists()) {
            String newfilename = "2_"+filename;
            response.setFilename(response.getResponse() + "Filename already exist, reset the filename to "+newfilename+"\n");
            data.setTitle(newfilename);
        }
        
        ExcelFile file = new ExcelFile(id++, data);
    ...
    public void downloadExcel(@PathVariable int id, HttpServletResponse response) throws IOException {
    ...
    public ResponseEntity<ExcelResponse> deleteExcel(@PathVariable int id) {
